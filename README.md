...
**On Linux**, the test suite currently has 116 registered cases in shared mode and 114 in static mode. These two numbers deliberately exclude `preci_selftest` (`tests/CMakeLists.txt`), a local lint self-test that only registers when clang-format, clang-tidy and cppcheck are all found on the machine that ran `cmake`: a raw `ctest --test-dir build -N` on a machine that has all three reads one higher than each number above, and matches them exactly on a machine that does not. The difference between the two modes is the pair `visibility_test`/`visibility_selftest`, which only makes sense against a shared object and never registers in static mode.

**On Windows**, the test suite currently has 114 registered cases in shared mode and 114 in static mode. The two modes match because the Windows counterpart of that pair (`exports_win_test`/`exports_win_selftest`) is not gated off in static mode.

**How the two platforms relate, and who checks it.** The four totals above are the only figures this section states, and a CI gate (`readme_test_count_test`, `tests/tools/check_readme_test_count.py`) compares each of them against a live `ctest -N` on the matching platform, so they cannot drift silently; a second gate (`readme_volatile_numbers_test`) reproves any other digit written into these paragraphs, so that no figure without a gate can appear here. Everything below is structure, and the structure is checked by a third gate. The two platforms do not register the same names, and they are not required to: what is required (this project's `GODS_LAWS.md`, law L-04) is that no verification vanishes on one side. A case that exists under one name on Linux and under another on Windows, proving the same behaviour through that platform's own mechanism (the consumption, embedding, install-layout, output-name, port-privacy, exports and dependency-zero gates all take this shape, with a `_win` suffix on the Windows side), is declared as a pair in `tests/parity_aliases.txt`. A case that exists on one side only, because what it checks has no counterpart on the other (a Wayland protocol binding, a Windows runner probe), is declared as an accepted absence in `tests/parity_exceptions.txt`, each with its reason and the tracking item that will close it, or a marker saying the absence is permanent by design. The `parity` job in `.github/workflows/ci.yml` downloads the real `ctest -N` inventory that every Linux and Windows job produced, unions them per system, and reproves any name present on one side and missing on the other without a line in one of those two files, as well as any accepted absence whose tracking item is already marked done. The honest statement of parity is therefore not a number: it is that job being green on the run you are looking at, plus those two files, which you can read directly. Everything that merely scans the source tree (licence headers, third-party purity, zero dependency, header hygiene coverage, layer violations, the forbidden graphics term, the laws-table duplication, public name collision, blank install-dir rejection, the pkg-config validator, macro balance in public headers, the environment-fact sweep, and this README count itself) runs on all five platforms. A gate that cannot exercise part of itself on one platform declares that part not applicable and prints how much it declared, rather than disappearing from the inventory or passing silently; `public_name_collision_test` and `dep_zero_selftest` are the two that do this on Windows today. Outside `ctest`, the `windows` job also runs PowerShell scripts under `tools/ci/` against the real compiled binary's own imports and exports; those checks are real and running, but they are not `ctest` cases and are not part of the totals above.
