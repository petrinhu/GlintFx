...
[[nodiscard]] declaration_parse_result accepted(gfss_declaration declaration) {
    return declaration_parse_result{.accepted = true, .declaration = std::move(declaration), .diagnostic = {}};
}
...
[[nodiscard]] name_and_colon_outcome read_name_and_colon(const std::vector<gltfx_gfss_token> &tokens,
    ...
    if (!span.has_content || tokens[span.first].kind != gltfx_gfss_token_kind::ident) {
        ...
        return name_and_colon_outcome{
            ...
            .diagnostic = gltfx_gfss_diagnostic{
                .line = tokens[at].line, .column = tokens[at].column, .expected = k_expected_property_name, .detail = {}}};
    }
    ...
    while (colon_index <= span.last && tokens[colon_index].kind == gltfx_gfss_token_kind::whitespace) {
        ++colon_index;
    }
...
[[nodiscard]] std::optional<gfss_universal_keyword> universal_keyword_for(std::string_view text) noexcept {
...
[[nodiscard]] universal_scan_outcome scan_for_universal_keyword(const std::vector<gltfx_gfss_token> &tokens,
                                                                std::size_t first, std::size_t last) {
    if (first == last && tokens[first].kind == gltfx_gfss_token_kind::ident) {
        if (const std::optional<gfss_universal_keyword> keyword = universal_keyword_for(tokens[first].lexeme)) {
            return universal_scan_outcome{.is_universal_alone = true, .keyword = *keyword};
        }
    }
    for (std::size_t i = first; i <= last; ++i) {
        if (tokens[i].kind == gltfx_gfss_token_kind::ident && universal_keyword_for(tokens[i].lexeme)) {
...
[[nodiscard]] declaration_parse_result build_shorthand(const std::vector<gltfx_gfss_token> &tokens,
                                                        std::string_view shorthand_name, bool important,
...
apply_contract_value(const std::vector<gltfx_gfss_token> &tokens, const trimmed_token_span &value_span,
                     const property_value_contract &contract, gfss_declaration &declaration) {
...
[[nodiscard]] declaration_parse_result build_known_property(const std::vector<gltfx_gfss_token> &tokens,
    ...
    if (universal.found_but_mixed) {
        ...
        return rejected(gltfx_gfss_diagnostic{
            .line = at.line, .column = at.column, .expected = k_expected_universal_keyword_alone, .detail = {}});
    }
...
declaration_parse_result parse_declaration(const std::vector<gltfx_gfss_token> &tokens, std::size_t begin,
                                           std::size_t end) {
    ...
    if (!value_span.has_content) {
        const std::size_t at = name_and_colon.value_begin < end ? name_and_colon.value_begin : begin;
        return rejected(gltfx_gfss_diagnostic{
            .line = tokens[at].line, .column = tokens[at].column, .expected = k_expected_component_value, .detail = {}});
    }

    if (name_and_colon.lookup.kind == property_name_lookup_kind::shorthand) {
        return build_shorthand(tokens, name_and_colon.lookup.shorthand_name, flag.important, value_span);
    }
