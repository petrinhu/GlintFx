...
**Nine versions have been tagged so far; the most recent is `v0.3.7.0`** (see "Status" above). Pin to the latest directly, or to any earlier tag (`git tag --list 'v*'` against a clone shows all nine) - `GIT_TAG` takes a tag name exactly the way it takes a commit SHA:
