...
**Nine versions have been tagged so far; the most recent is `v0.3.7.0`** (see "Status" above). Pin to the latest directly, or to any earlier tag (`git tag --list` against a clone shows all nine) - `GIT_TAG` takes a tag name exactly the way it takes a commit SHA:
...
See [`CHANGELOG.md`](CHANGELOG.md). Nine versions have been tagged so far, the most recent `v0.3.7.0` (see "Status" above), but the changelog has fallen behind again: its most recent dated section is `[0.3.0.0]`, and the seven tags published after it (`v0.3.1.0` through `v0.3.7.0`) have no entry there yet, with the compare and release links at the bottom of the file still pointing at `v0.3.0.0` too. That gap is tracked in [`TODO.md`](TODO.md), not silently accepted as done.
