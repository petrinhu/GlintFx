...
The test suite currently has 46 registered cases in shared mode and 45 in static mode (the difference is `visibility_test`, which only makes sense against a shared object). These two numbers deliberately exclude `preci_selftest` (`tests/CMakeLists.txt`), a local lint self-test that only registers when clang-format, clang-tidy and cppcheck are all found on the machine that ran `cmake` - most developer machines have them and so does the dedicated `lint` CI job, but the other CI targets build with a plain toolchain and never register it. A raw `ctest -N` total is therefore a property of the machine, not of the repository: this project measured it both ways on the same commit (47/46 with the three tools installed, 46/45 without) and shipped documentation once that stated the raw total from a machine that had them, which the `wayland-container` CI job - which does not - correctly reproved. Run `ctest --test-dir build -N` yourself to see your own raw total: it will be one higher than the numbers above if your machine has clang-format/clang-tidy/cppcheck, and match them exactly if it does not. The CI gate that checks this sentence (`readme_test_count_test`, see `tests/tools/check_readme_test_count.sh`) makes the same subtraction before comparing, so it holds on every target, not just the one it was written from.
