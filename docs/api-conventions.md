...
| **Produção** (`CMAKE_BUILD_TYPE=Release`, ou qualquer compilação com `-DNDEBUG` — o padrão deste projeto para CI/`preci.sh`) | Custo zero: a guarda de depuração vira `((void)0)`. O que sobra depois disso é comportamento indefinido — **continua sendo**, em nenhuma leitura isto é uma garantia — mas desde 25/08/2026 as duas formas do envelope (`gltfx_rslt<T>` e a especialização `gltfx_rslt<void>`) compartilham a MESMA representação interna (`std::variant`), então "falhar alto" (desreferenciar um ponteiro nulo de verdade, `SIGSEGV`, porque a página zero é desmapeada nos cinco alvos deste projeto) é **estrutural** nas duas formas, não só em uma. **"Estrutural" não é "garantido pelo padrão"**: é uma propriedade de COMO o código está escrito hoje, que viaja com o código; a FORMA exata da queda (que sinal, se alguma camada de endurecimento do compilador intercepta antes) viaja com o compilador e o sistema operacional, não com o glintfx. | Nenhuma mensagem do glintfx no `stderr`. Se o processo travar (o caso comum, nos cinco alvos, para as duas formas), a mensagem não vai citar `gltfx_rslt` nem `has_value`/`has_error` — pode ser um `SIGSEGV` cru, ou, especificamente em build **sem otimização** (`-O0`, qualquer plataforma com toolchain GNU — GCC, ou Clang usando `libstdc++`), uma verificação do PRÓPRIO `libstdc++` chegando primeiro. **CORREÇÃO de 26/08/2026, a causa estava errada até aqui:** a versão anterior deste texto atribuía isto a *"endurecimento do libstdc++ do Fedora"*, como peculiaridade de distribuição — **não é**, e a causa real foi medida nesta sessão, não presumida. O mecanismo é `_GLIBCXX_ASSERTIONS`, e o próprio cabeçalho da biblioteca padrão (`bits/c++config.h`) o religa sozinho sempre que a macro de compilador `__OPTIMIZE__` não está definida, e essa macro só nasce com `-O1` ou mais — **independente de qualquer `-D`/`-U` explícito na linha de comando** (medido nesta sessão, GCC 16.2.1, um `std::optional` desengatado, nos quatro níveis: religa em `-O0` mesmo passando `-U_GLIBCXX_ASSERTIONS`; fica desligado de `-O1` para cima, salvo `-D_GLIBCXX_ASSERTIONS` explícito). É comportamento do `libstdc++` **upstream** (GNU), não um patch do Fedora, e por isso aparece em **qualquer** toolchain GNU/`libstdc++` compilado sem otimização, em qualquer uma das cinco plataformas onde esse par (GNU + `libstdc++`) existir — nenhuma das duas formas de queda é uma garantia do glintfx. |
