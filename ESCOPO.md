...
2. **Unidades** — quatro unidades de ângulo (`deg rad grad turn`), seis de tempo (`ms s min h ns frames`, as cinco grafias do padrão mais `frames`), e as dezesseis de comprimento (as doze antigas mais `ch lh vmin vmax`).
   ...
   ⚠️ **CORREÇÃO (28/08/2026, mesma sessão, mais tarde no dia):** a frase original deste item — *"cada família teve vermelho por execução real (falha de asserção, não só falha de compilação) antes do verde correspondente"* — **é falsa para a sub-parte ângulo/tempo**. O registro citado como prova (`/var/tmp/glintfx-plan/red2-units.log`, carimbo 08:54:02) mostra erro de **compilação** (`'gltfx_gfss_angle_unit' does not name a type`) — e o commit `e579ad6`, de 08:53:40, **vinte e dois segundos ANTES**, já continha esse mesmo tipo, com todos os membros, no header. O vermelho registrado contradiz o que já estava commitado; não sustenta a alegação de "vermelho por execução real" para ângulo/tempo. A alegação sobre a sub-parte **comprimento** (12→16) não foi reexaminada por esta correção — permanece como estava, sem endosso nem contestação.
