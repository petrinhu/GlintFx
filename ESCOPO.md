...
**Teclado — RESOLVIDO por ele em 06/09/2026, por pergunta direta, depois de o CTO nomear a ambiguidade em vez de presumir.** Havia duas leituras, e uma delas **revogaria a lei de 21/08** (parser próprio) trazendo de volta a dependência que ela tirou. **Resposta dele: o parser CONTINUA.** A trilha `KEYMAP-*` é o **leitor** do que o sistema informa — **parser não é detector**. ⚠️ **E "não precisa novo detector" vira PROIBIÇÃO EXPLÍCITA de adivinhar:** identificar teclado físico, deduzir layout por hardware ou região, embutir mapa próprio quando o sistema já deu um, e enumerar teclados. **Nada disso estava planejado** — o escopo não muda, só ganha as proibições escritas.

**Mouse:** a biblioteca entende **o maior número possível de botões que o sistema registrar nativamente**.

⚠️ **A fronteira, ESCLARECIDA por ele em 06/09/2026, e o esclarecimento muda o desenho.** Verbatim: *"se o app do mouse fizer o SO expor mais botoes sem que glintfx precise enxergar esse app, tudo bem. Mas nossa lib não pode ligar diretamente ao app do fabricante do mouse. Isso não faz sentido."* **Ou seja: a linha não é onde os botões nascem, é COM QUEM a biblioteca fala.** Se o programa do fabricante ensina o sistema a expor dez botões, a biblioteca lê os dez pelo caminho normal do sistema e está tudo certo — ela nem sabe que aquele programa existe. **O proibido é a biblioteca conversar com o programa do fabricante**: nada de biblioteca específica de marca, protocolo proprietário ou detecção de software de terceiro. **A biblioteca fala com o sistema, e só.**
...
✅ **DECIDIDO pelo CTO em 06/09/2026 (D-W6b-16 a 21):** o número de opções cresce **sem congelar N campos fixos**. A superfície congela **a forma de pedir uma opção e de perguntar se ela existe neste sistema**; as opções entram por lista que **só cresce**, como o registro de propriedades de estilo desta casa já faz. ⚠️ **E a parte que resolve a dor de verdade:** pedir opção que **não existe neste sistema** é **recusado pelo nome**, nunca degradado em silêncio — veio de dor relatada, em biblioteca onde pedir sincronia adaptativa **falha e manda tentar outro valor** sem dizer o que houve. **Toda opção declara QUANDO vale** (só na abertura, ao vivo, ou só leitura), para o menu do consumidor rotular "exige reiniciar" **sem manter tabela própria**.
...
✅ **DECIDIDO pelo CTO em 06/09/2026 (D-W6b-13/14/15), e reconfirmado por pesquisa de mercado e de dores:** a versão 1 **informa**, não escolhe. O consumidor pergunta o tipo de placa em uso e o nome do renderizador; **`desconhecido` é resposta legítima** ("o sistema não disse"), nunca chute. ⚠️ **Escolher ficou de fora com razão medida, e a pesquisa a endureceu:** no Windows a escolha depende de símbolo exportado **pelo executável do consumidor** — dentro de uma biblioteca compartilhada **não funciona**, e há relato de quem ficou sem saída por isso — e desde 2020 o próprio sistema sobrepõe; no Linux, o mecanismo que o mercado usa **falha justamente sob Wayland**, e o modo de falha dele é **cair em software sem avisar**. **A porta fica reservada** no descritor de abertura, aceitando só "sem preferência" e **recusando o resto pelo nome**, até existir a fatia que a honre.
...
2. **Unidades** — quatro unidades de ângulo (`deg rad grad turn`), seis de tempo (`ms s min h ns frames`, as cinco grafias do padrão mais `frames`), e as dezesseis de comprimento (as doze antigas mais `ch lh vmin vmax`).

   ⚠️ **CORREÇÃO (28/08/2026, mesma sessão, mais tarde no dia):** a frase original deste item — *"cada família teve vermelho por execução real (falha de asserção, não só falha de compilação) antes do verde correspondente"* — **é falsa para a sub-parte ângulo/tempo**. O registro citado como prova (`/var/tmp/glintfx-plan/red2-units.log`, carimbo 08:54:02) mostra erro de **compilação** (`'gltfx_gfss_angle_unit' does not name a type`) — e o commit `e579ad6`, de 08:53:40, **vinte e dois segundos ANTES**, já continha esse mesmo tipo, com todos os membros, no header. O vermelho registrado contradiz o que já estava commitado; não sustenta a alegação de "vermelho por execução real" para ângulo/tempo. A alegação sobre a sub-parte **comprimento** (12→16) foi medida à parte, mesma sessão, ver bloco "CORREÇÃO 2" logo abaixo do quarto par vermelho-verde: ao contrário de ângulo/tempo, o ciclo dela SUSTENTA (cronologicamente sólido, sem contradição com o que já estava commitado) — mas a frase geral erra a caracterização dela também, por outro motivo.

   **Refeito honestamente nesta mesma data**, em worktree isolado (`/var/tmp/glintfx-red2-redo`, a partir de `e579ad6`, nunca tocando a árvore compartilhada), em quatro pares vermelho-verde reais, cada um com timestamp antes/depois provando a ordem:
   - **Ângulo, comportamento de parsing** — teste que só referencia os tipos (não `name()`), red genuíno por asserção (`deg.ok` falso, `/var/tmp/glintfx-plan/redo-A1-angle-parse-RED.log`, 09:58:27) antes da tabela de busca entrar em `value_parse.cpp` (`redo-A2-angle-parse-GREEN.log`, 09:59:10).
   - **Tempo, comportamento de parsing** — mesma forma, red genuíno (`ms.ok` falso, `redo-B1-time-parse-RED.log`, 09:59:37) antes da tabela de tempo (`redo-B2-time-parse-GREEN.log`, 10:01:25).
   - **Round-trip `name()`** — vermelho aqui é honestamente de **LINK** (`undefined reference to gltfx_gfss_angle_unit_name`/`..._time_unit_name`, `redo-C1-red-build.log`, 10:01:55), não de asserção — declarado como tal, não maquiado; verde após `value_kind.cpp` implementar as duas funções (`redo-C2-name-GREEN.log`, 10:02:31).
   - Cobertura fechada (L-40, todas as 4 + todas as 6 unidades), teste dedicado de `frames` sem conversão de 60 Hz, teste de colisão `min`/`min()`, e o guardrail de isolamento entre as três naturezas foram acrescentados por cima do mecanismo já verde, sem vermelho fabricado (o guardrail de isolamento, em particular, é consequência de desenho, não comportamento novo — mesma honestidade que o item 3 abaixo já registrava para esse caso). Suíte completa da árvore isolada: 58/58 (`redo-D2-final-ctest-shared.log`, encerrado 10:0x).
   
   Todos os logs citados ficam em `/var/tmp/glintfx-plan/` (scratch, não versionado) como evidência; o código de produção real (`60b23f5`) **não foi alterado** por esta correção — ele já estava certo, aprovado por duas revisões e por mutação. O que estava errado era só o registro de como o ciclo TDD daquela sub-parte foi (não foi) provado.

   ⚠️ **CORREÇÃO 2 (28/08/2026, mesma sessão, ainda mais tarde no dia) — a sub-parte comprimento (12→16), fechando a lacuna que a CORREÇÃO acima deixou explicitamente aberta.** Medida pelo mesmo método: o que o commit `e579ad6` já continha, quando o registro foi escrito, e se as duas coisas são compatíveis.

   **Resultado: o ciclo SUSTENTA.** Ao contrário de ângulo/tempo, `git show e579ad6:include/glintfx/gfss/value.hpp` confirma que o enum `gltfx_gfss_length_unit` tinha, naquele commit, só os 12 membros ORIGINAIS (`px dp em rem ex vw vh in cm mm pt pc`) — `ch`, `lh`, `vmin`, `vmax` **não existiam**. `git show e579ad6:tests/gfss_value_test.cpp` confirma que o arquivo de teste, no mesmo commit, **também não referenciava** nenhum dos quatro. O registro `/var/tmp/glintfx-plan/red4-lengthunits.log` (carimbo 08:57:42, **depois** de `e579ad6` às 08:53:40 e **antes** de `60b23f5` às 09:06:39) mostra erro de compilação genuíno — `'ch' is not a member of 'glintfx::style::gltfx_gfss_length_unit'` — que só é possível se o teste foi editado para referenciar os quatro novos membros **antes** de o enum ganhá-los, exatamente a ordem que a L-20 exige. `green4-lengthunits.log` (08:58:53) mostra o mesmo teste passando depois de `value.hpp`/`value_parse.cpp`/`value_kind.cpp` ganharem os quatro membros e a fiação correspondente. `full-build-after-angle-time.log` (08:57:11, um checkpoint de build completo logo antes do red) e a sequência `full-build-units.log`/`full-ctest-units.log`/`static-build-units.log`/`static-ctest-units.log` (08:59 a 09:06, todos antes do commit) mostram o trabalho de comprimento acontecendo numa janela contígua, sem lacuna nem log fora de ordem. **Nenhuma contradição encontrada; nenhuma refação necessária** — diferente de ângulo/tempo, aqui não há worktree isolado nem par vermelho-verde novo para registrar, porque o par antigo já é honesto.

   **Mas a frase geral erra a caracterização mesmo assim, só que por outro motivo.** "Falha de asserção, não só falha de compilação" **não descreve** o que `red4-lengthunits.log` de fato mostra: é uma falha de **compilação** (`'ch' is not a member of...`), não uma asserção em tempo de execução (`result.ok` falso). Não é fabricação nem viola a ordem cronológica — é a MESMA classe de vermelho que a etapa 1 (linha acima, "Naturezas") já usa sem disputa, e a mesma classe que a CORREÇÃO acima documentou como honesta para o round-trip `name()` de ângulo/tempo. A lei L-20 pede "execute e veja falhar", e um `ninja` que para no `cc1plus` com erro de compilação **é** uma execução que falhou — a linha exigida por L-20 está do lado de dentro, não do lado de fora. A frase geral só erra ao alegar UNIFORMEMENTE, para as três famílias, um tipo de vermelho ("falha de asserção, não só falha de compilação") que nenhuma das três efetivamente teve nesta forma: naturezas foi compilação (documentado, sem disputa), comprimento foi compilação (agora confirmado), e só ângulo/tempo teria sido assertivo — se o registro citado para eles não fosse, ele próprio, impossível pela ordem do commit (ver CORREÇÃO acima).

   **Conclusão desta segunda correção:** endosso a sub-parte comprimento como ciclo TDD genuíno e cronologicamente sólido; corrijo apenas a frase-guarda-chuva do item 2, que generalizava "falha de asserção" para as três famílias quando duas delas (comprimento e, na descrição correta, naturezas) sempre foram de compilação. Nenhum código de produção foi tocado por esta segunda correção — o achado é só sobre o registro.
...
### Decisão 16 - a família do erro copiado se conserta ANTES da onda dos 140

**Origem:** ordem do líder em 18/09/2026, por `AskUserQuestion`, escolhendo "Antes dos 140" diante da medição abaixo. A razão que ele escolheu, e que fixa a prioridade: **esta família mata em todos os cinco sistemas, inclusive em Release; a família A só mata no modo de depuração da Microsoft.**

**O defeito, na forma exata:** `gltfx_rslt<T>::err(gltfx_err failure)` recebe o parâmetro **por valor** (`include/glintfx/core/err.hpp:299` e `:391`). Quando o argumento é um erro que já existe - a forma `::err(alguma_coisa.err())` -, montar esse parâmetro invoca o construtor de cópia, e o construtor de cópia **aloca** quando a origem carrega diagnóstico anexado (`src/core/err.cpp:33`, `m_context = new err_context(*other.m_context)`, `new` que lança, com um `std::string` dentro). Dentro de função marcada `noexcept` sem `try` cobrindo, a falta de memória escapa e derruba o processo do consumidor. É o mesmo defeito que `FIX-OOM-B9` já consertou à mão em `src/platform/gl/gpu_enumeration_facade.cpp`, e o conserto de lá é o idioma já aprovado neste projeto.

**O universo medido em 18/09/2026 19:28 pelo orquestrador, contra `7a450aa`:** 174 chamadas `::err(` em `src/`; **27** copiam um erro que já existe; 142 constroem um erro novo no lugar (sem cópia); 2 copiam variável local nomeada e são justamente as duas já consertadas. O acessor `.err()` devolve `const gltfx_err &`, então há **exatamente uma** cópia por sítio - a do parâmetro. Anexar diagnóstico é a norma e não a exceção: **29 arquivos** de `src/` chamam `with_rejected_value`/`with_byte_offset`/`with_os_error_code`.

**Um sítio conferido à mão, de ponta a ponta, para que a decisão não repouse em estimativa:** `src/platform/wayland/seat_adapter.cpp:88` devolve `::err(seat_proxy.err())` dentro de `wayland_seat_adapter::open(...) noexcept` (linha 73), **sem `try` nenhum na função**, e o erro de origem vem de `connection.bind(...)`, que carrega diagnóstico.

**O que isto revela sobre o portão, e é a terceira vez em dois dias:** `tests/tools/check_noexcept_alloc.py` devolve `familia_B_achados=0` contra a árvore inteira, com o sítio acima presente. O portão já declara duas cegueiras no próprio cabeçalho (construção que não é direta de `vector`/`string`; cópia por atribuição). **Esta é a terceira**, e o plano da onda tem de ensiná-lo a ver esta forma, com estreia vermelha obrigatória contra o defeito plantado.

**O que NÃO está medido, declarado e não escondido:** quantos dos 27 são defeito de verdade. A minha classificação heurística disse 14 dentro de função `noexcept`, dos quais 5 já têm `try`, restando **9 candidatos** - mas o instrumento é uma expressão regular que procura a assinatura envolvente subindo no texto, **não é um analisador de C++**, e erra em template, em função aninhada e em ramo por plataforma. Os sítios de `src/platform/loop/loop_engine.hpp` são template e a heurística os classificou como não-`noexcept`; não confio nisso. Medir com instrumento defensável é o primeiro passo da onda, não uma etapa opcional dela.

**Consequência para a Decisão 15:** a ordem dos quatro passos dela continua inteira e não é revogada. Só o **momento** muda: esta onda entra na frente.

### Decisão 17 - o erro passa a carregar o diagnóstico COMPARTILHADO, e isso mata os 129 de uma vez

**Origem:** ordem do líder em 18/09/2026, por `AskUserQuestion`, diante da medição que derrubou a minha. Ele escolheu a opção recomendada pelo planejador: **mudar como `gltfx_err` carrega o contexto** (contagem de referência intrusiva dentro de `err_context`, com cópia-na-escrita), em vez de proteger sítio a sítio ou de pôr `try`/`catch` no cabeçalho público.

**A causa, medida e provada mecanicamente, e ela é uma só:** os anexadores de contexto (`with_path`, `with_position`, `with_byte_offset`, `with_rejected_value`, `with_os_error_code`) devolvem **`gltfx_err &`** - referência de lvalue, não prvalue (`include/glintfx/core/err.hpp:254-258`). Logo a forma idiomática `::err(gltfx_err(codigo).with_rejected_value(valor))` liga um **lvalue** ao parâmetro por valor de `err(gltfx_err failure)`, e isso chama o **construtor de cópia**, sem elisão possível. E como o `with_*()` acabou de criar o contexto, essa cópia aloca em **100%** das execuções, não condicionalmente.

**Prova independente, com controle nos dois sentidos, feita pelo orquestrador em 18/09/2026 19:47** (não é raciocínio, é o compilador respondendo): tipo mímico com construtor de cópia `= delete`, mesmo formato do real (anexador devolvendo referência, `err()` recebendo por valor), sob `g++ -std=c++23 -fsyntax-only`. **Controle A**, prvalue puro `err(mimico(7))`: compila, código 0. **Caso B**, cadeia `err(mimico(7).com_valor("x"))`: reprova com `error: use of deleted function 'mimico::mimico(const mimico&)'`, código 1. A cópia é obrigatória e o compilador a exige.

**O universo real, medido pelo planejador contra instrumento calibrado nos dois sentidos, e que DERRUBA a medição do orquestrador registrada na Decisão 16:** não são 27 sítios de cópia nem 9 defeitos. São **179 sítios** com argumento (`src/` mais `include/`), **137** que copiam, e **129 defeitos reais** em **27 arquivos**. Distribuição: `win32` 46, `wayland` 39, `gl` 18, `loop` 18, `window` 8; zero em `core/` e `gfss/`. **O erro da minha medição, nomeado para não se repetir:** classifiquei as 102 chamadas da forma `::err(gltfx_err(c).with_*(v))` como "constroem novo no lugar, prvalue, sem cópia". A cadeia `.with_*()` converte o prvalue em lvalue, e foi exatamente a metade **pior** do universo que eu dei como segura.

**O que a Decisão 17 congela, e é o preço dela:** (1) a cópia de `gltfx_err` passa a ser `noexcept` e sem alocação, **para sempre**; (2) `gltfx_err` vira tipo de valor com contexto **compartilhado** até alguém escrever nele; (3) a contagem é atômica, então copiar erro entre linhas de execução é seguro e **não pode ser rebaixado depois**.

**O que NÃO congela, medido:** a pegada. `sizeof(void*)=8`, hoje `16`, com a decisão `16`. A variante com `std::shared_ptr` daria `24` e **quebraria** o `static_assert(sizeof(gltfx_err) == 2 * sizeof(void *))` de `err.hpp:279` - por isso está fora, e a contagem intrusiva está dentro.

**As duas alternativas rejeitadas, com a razão:** pôr `try`/`catch` no cabeçalho público conserta tudo num lugar só, mas **proíbe o consumidor de compilar sem exceções** (medido com controle: código 0 com exceções, código 1 e `error: exception handling disabled` sem) - perda permanente de portabilidade decidida em nome de um consumidor que este projeto, por definição, não pode consultar. Repetir a proteção nos 129 sítios são cerca de 780 linhas de ruído em 27 arquivos, perde diagnóstico em silêncio sob falta de memória, e cria 129 proteções para vigiar indefinidamente. **Escada de recuo, se a decisão precisar ser revista:** compartilhado, depois cabeçalho público, depois sítio a sítio.

**Uma família irmã vem de brinde:** `operator=(const gltfx_err &)` (`err.hpp:210`) também não é `noexcept` e também aloca, e o portão `check_noexcept_alloc.py` já confessa no próprio cabeçalho ser cego a cópia por atribuição. A decisão conserta os dois.

**Correção de fato registrada na Decisão 16:** onde ela diz "27 copiam um erro que já existe" e "9 candidatos", leia **137 copiam** e **129 defeitos**. A Decisão 16 continua válida no que ela decide (a ordem: esta família antes da família A); só o número dela estava errado, e o erro era meu.

**Nenhum símbolo exportado muda de nome, e isso está PROVADO nos cinco alvos.** O Itanium não codifica especificação de exceção no *mangling*, o que cobria os quatro alvos de Linux por construção. **O Windows, que era a única lacuna declarada desta decisão, foi fechado em 18/09/2026 pela fatia `ERR-COPY-TWINS`, NESTA MÁQUINA e não no servidor:** `glintfx.dll` construída com `cl.exe`/`link.exe` reais no container `tools/msvc-container/`, em duas árvores - a atual e uma cópia descartável no commit `612e59f`, anterior ao conserto -, e `dumpbin /exports` comparado byte a byte nas duas: **98 símbolos exportados dos dois lados, diferença zero**, incluindo o próprio construtor de cópia (`??0gltfx_err@glintfx@@QEAA@AEBV01@@Z`). **Proveniência, porque importa:** a prova é do agente da fatia F5, no container MSVC; o orquestrador conferiu o relatório e a forma, e a re-execução independente fica para a revisão adversarial da onda.

**Correção de uma afirmação do planejador, conferida contra o disco em 18/09/2026 19:49:** ele escreveu que os 46 sítios de `win32/` "não são exercíveis nesta máquina". **São.** A máquina virtual Windows 11 existe em `/var/tmp/glintfx-win-lab/` (23 GB), e o container do compilador da Microsoft existe em `tools/msvc-container/`. Este é o mesmo esquecimento que o orquestrador cometeu em 17/09 e que o líder corrigiu; a prova do Windows não depende só do servidor.
