...
# Prints one path per line, relative to <include_dir>, for every *.hpp
# found under it - e.g. "glintfx/core/version.hpp" when <include_dir>
# is the project's top-level include/. Empty output (not an error by
# itself here) means the caller found nothing; require_nonempty_scan
# below is the floor that turns that into a hard failure.
enumerate_public_headers() {
    include_dir="$1"
    find "$include_dir" -type f -name '*.hpp' 2>/dev/null | while IFS= read -r f; do
        printf '%s\n' "${f#"$include_dir"/}"
    done | sort
}
...
header_is_included_in_tu() {
    header="$1"
    tu_file="$2"
    grep -qF "#include <${header}>" "$tu_file"
}
...
selftest_main() {
    ...
    echo "check_hygiene_coverage.sh --selftest: os tres controles OK"
}
