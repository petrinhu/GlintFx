...
int main() {
    ...
    glintfx::container_fixture::checked_fprintf(
        stdout, "window_active_after_map_smoke: mapped (3 frames swapped)\n");
    ...
    glintfx::container_fixture::checked_fprintf(
        stdout, "MEASURED window_active_after_map_smoke.roundtrips=%d\n", roundtrips_used);
    ...
    glintfx::container_fixture::checked_fprintf(
        stdout, "window_active_after_map_smoke: OK - window.state(active) verdadeiro dentro do "
                "orcamento\n");
    return EXIT_SUCCESS;
}
