...
// core/error_code.hpp - the numeric identity of every error glintfx can
...
// value is ever retired, its row stays in the table
// (error_code.cpp), documented as retired; the number is never given
// to a new meaning.
//
...
//              table (see error_code_name() below - the fallback for a
//              value produced by a NEWER glintfx than the one reading
//              it; graceful degradation, never undefined behavior)
...
// through ONE table in error_code.cpp, not a switch that grows a case
// per feature. This header only declares the enumerators (the values
// themselves) and the single exported lookup function.
//
// SYSTEM-MACRO COLLISION CHECKLIST (GODS_LAWS.md L-19/CORE-ERROR,
// mandatory before closing this slice - the same class of bug already
// bit glintfx/core/version.hpp's major/minor fields, see that header):
// every identifier below was checked against the function-like macros
// known to collide on this project's five platforms - min/max, ERROR,
// DELETE, IN, OUT, CONST, VOID, TRUE, FALSE, interface, small, near,
// far, STRICT (the Windows system headers, named in prose rather than
// as literal #include text on purpose - see the same rationale spelled
// out in version.hpp - because tests/tools/check_layers.sh greps this
// file for OS-header patterns and would trip a false positive on a
// comment that merely names them), and major/minor/makedev,
// stdin/stdout/stderr, unix/linux (glibc/POSIX headers, Linux legs).
// `error_code`, `unknown`, `out_of_memory`, `io_failure`, `not_found`,
// `invalid_argument`, `parse_failure`, `unsupported`,
// `platform_failure` and `error_code_name` match none of them.
...
enum class error_code : std::uint32_t { // NOLINT(performance-enum-size) reason: see the paragraph
                                        // above; 32 bits is a frozen ABI/plan decision, not an
                                        // oversight
    unknown = 0,

    // Generic band (1..999). v1 carries exactly these seven; adding an
    // eighth means adding both the enumerator here and its row in
    // error_code.cpp's table - the two never drift because the table
    // is the only place a name is produced from a value.
    out_of_memory = 1,
    io_failure = 2,
    not_found = 3,
    invalid_argument = 4,
...
// as error_code::unknown itself.
[[nodiscard]] GLINTFX_API std::string_view error_code_name(error_code code) noexcept;
