...
**Windows (MSVC) was undocumented here until 2026-09-03 (RSLT-PARITY-WIN) — it now has a real gate, but STILL not a real measurement of the two facts below, corrected on 2026-09-04 after the gate's own estreia.** `tests/tools/check_rslt_precondition.py` (the cross-platform replacement for the old POSIX-sh script, see its own header) compiles and runs the SAME two precondition violations under MSVC too, on every CI push. What it asserts there, by Microsoft's own documentation rather than by guess: the Debug message format is `"Assertion failed: <expression>, file <file>, line <line>"` (`_wassert`, `<cassert>`), and the expression text includes the same literal string this project's own `assert()` calls carry, so the same substring check works unchanged. The Production/`gltfx_rslt<void>` structural fault is expected to be an unhandled `STATUS_ACCESS_VIOLATION` (`0xC0000005`), Windows' equivalent of "page zero unmapped, dereferencing a genuine null pointer" — reported back as exit status `-1073741819`, the SAME NT status read as a signed 32-bit integer. **Still NOT observed on a real Windows toolchain** — the estreia (CI run `33832169390`, 2026-09-04) reached this fixture on real Windows for the first time, but never got far enough to measure either fact: the script itself hardcoded the literal `/std:c++23` for MSVC, and no MSVC version has ever accepted that literal — `cl` emitted `Command line warning D9002 : ignoring unknown option '/std:c++23'`, silently fell back to its pre-C++17 default, and BOTH compiles (`Debug`/`Release`) of `precondition_fixture.cpp` failed outright with a cascade of `<string_view>`/`<variant>` not being members of `std` — the fixture never linked, let alone ran. This was a bug in the GATE, not in the library or its precondition guard: `tests/CMakeLists.txt` now passes `CMAKE_CXX23_STANDARD_COMPILE_OPTION` (CMake's own per-compiler resolution of the C++23 flag — `-std:c++latest` on the toolset the estreia measured, `MSVC 14.51.36231`/"Visual Studio 18 Enterprise") instead of a literal either script had to guess, matching the flag the SAME job's real library build already uses successfully through `cmake/GlintfxCompileOptions.cmake`'s `CXX_STANDARD 23` property. Microsoft's own documentation still names a real risk the four POSIX targets never had: "a dialog box is always displayed following an assert call in debug mode" (learn.microsoft.com/cpp/c-runtime-library/reference/assert-macro-assert-wassert), which would hang an unattended CI runner instead of failing fast. The script mitigates this from outside the fixture (`SetErrorMode`, inherited by the child process) and bounds every run with a timeout as a safety net, but does not claim the mitigation is proven. **What is now fact, not hypothesis:** the gate reaches real `cl.exe` and the fixture compiles under the right standard (verified on Linux only, no Windows toolchain on the machine that made this fix — GODS_LAWS.md L-04). **What is still NOT fact:** the exact Debug assertion text and the exact access-violation exit code on Windows — those remain exactly what this paragraph asserted before, unmeasured; the next real Windows CI run against this fix is what turns them into fact, one way or the other.
