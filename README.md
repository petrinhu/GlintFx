...
**On Linux**, the test suite currently has 102 registered cases in shared mode and 100 in static mode (the difference is `visibility_test`, which only makes sense against a shared object). These two numbers deliberately exclude `preci_selftest` (`tests/CMakeLists.txt`), a local lint self-test that only registers when clang-format, clang-tidy and cppcheck are all found on the machine that ran `cmake` - most developer machines have them and so does the dedicated `lint` CI job, but the other CI targets build with a plain toolchain and never register it. A raw `ctest -N` total is therefore a property of the machine, not of the repository: this project measured it both ways on the same commit (103/101 with the three tools installed, 102/100 without). Run `ctest --test-dir build -N` yourself to see your own raw total: it will be one higher than the numbers above if your machine has clang-format/clang-tidy/cppcheck, and match them exactly if it does not.

**On Windows**, the test suite currently has 104 registered cases in shared mode and 104 in static mode. The static figure is measured straight off GHA run 34003230176 (05/09/2026, `ctest --test-dir build-static -N`); the shared figure is a DEDUCTION, not an independent measurement - that same run's shared-mode job never reached `ctest` at all, because five test executables failed to link there first (X-2/WIN-WINDOW fatia 9's own review, GODS_LAWS.md L-04, fixed in the same change that wrote this sentence). The deduction: the 97/97 this section stated before (measured against GHA run 33986752839, 05/09/2026 - the earlier 71/71 predates the PARITY-GATE work that moved most of the raw PowerShell consumption/boundary scripts into real `ctest` cases on Windows) plus the seven `glintfx_add_test()` calls added since then - one platform-agnostic (`display_backend_port_concept_test`) and six Windows-only (`win32_display_refusal_test`, `two_displays_test`, `win32_seat_translation_test`, `seat_test`, `win32_message_translation_test`, `win32_window_close_request_test`) - none of them gated by `BUILD_SHARED_LIBS`, so the same +7 applies to both modes. The next green run of the shared-mode job re-measures this number for real, through the same `readme_test_count_test` gate that caught the earlier drift. Shared mode is now at EXACT parity with Linux's 90: every Linux-only case pairs against a Windows-only case of matching shape - ten renamed pairs (`consume_test`/`consume_win_test`, `embed_test`/`embed_win_test`, `install_includedir_test`/`install_includedir_win_test`, `install_packager_layout_test`/`install_packager_layout_win_test`, `no_target_collision_test`/`no_target_collision_win_test`, `output_name_test`/`output_name_win_test`, `port_privacy_test`/`port_privacy_win_test`, `port_privacy_selftest`/`port_privacy_win_selftest`, and `visibility_test`/`visibility_selftest` renamed to `exports_win_test`/`exports_win_selftest`), one two-for-two SUBSTITUTION (`wl_proto_link_test` and `display_connect_failure_test` leave, `win32_display_connect_test` and `win32_runner_probe_test` enter), and one three-for-three set of DIFFERENT checks that still net to zero (`lib_no_undef_test`, `lib_no_undef_selftest` and `pkgconfig_test`, Linux-only, against `dep_zero_binary_win_test`, `dep_zero_binary_win_selftest` and `preci_win_selftest`, Windows-only). Static mode is NOT symmetric the same way: Windows still reports 90 (its `exports_win_test`/`exports_win_selftest` pair is not gated off in static mode), while Linux drops to 88 (`visibility_test`/`visibility_selftest` require `BUILD_SHARED_LIBS=ON` and never register in static mode at all) - so in static mode Windows reports two MORE cases than Linux, not fewer. Everything that merely SCANS the source tree now runs on all five platforms (`GATE-TREE-PARITY`, GODS_LAWS.md L-04): licence headers, third-party purity, zero dependency, header hygiene coverage, layer violations, the forbidden graphics term, the laws-table duplication, public name collision, blank install-dir rejection, the pkg-config validator itself, macro balance in public headers, the environment-fact sweep, and this very README count. Two gates run on Windows while DECLARING part of themselves inapplicable there, rather than vanishing from the inventory: `public_name_collision_test` prints `1 case, 0 exercised, 1 declared not applicable` where the GCC/Clang-only header discovery has no MSVC equivalent, and `dep_zero_selftest` declares three of its forty controls not applicable where they would need an ELF-only toolchain - declared and COUNTED absences, never silent skips. Outside `ctest`, `.github/workflows/ci.yml`'s `windows` job also runs dedicated PowerShell scripts (`tools/ci/check-embed.ps1` and the "Consumo instalado"/"Consumo embutido" steps, plus dependency-zero and port-privacy checks against the real compiled binary's own imports and exports) that cover, on that platform, the same consumption and boundary claims the Unix-only `ctest` gates prove through it on Linux - those checks are real and running, they are just not counted in the two numbers above because they are not `ctest` cases.
