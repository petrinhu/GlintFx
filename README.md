...
**On Linux**, the test suite currently has 70 registered cases in shared mode and 69 in static mode (the difference is `visibility_test`, which only makes sense against a shared object). These two numbers deliberately exclude `preci_selftest` (`tests/CMakeLists.txt`), a local lint self-test that only registers when clang-format, clang-tidy and cppcheck are all found on the machine that ran `cmake` - most developer machines have them and so does the dedicated `lint` CI job, but the other CI targets build with a plain toolchain and never register it. A raw `ctest -N` total is therefore a property of the machine, not of the repository: this project measured it both ways on the same commit (71/70 with the three tools installed, 70/69 without). Run `ctest --test-dir build -N` yourself to see your own raw total: it will be one higher than the numbers above if your machine has clang-format/clang-tidy/cppcheck, and match them exactly if it does not.

**On Windows**, the test suite currently has 58 registered cases in shared mode and 58 in static mode. The Windows total is still lower than the Linux one, and still identical between shared and static, but for a much smaller reason than it used to be. Every gate that merely SCANS the source tree - licence headers, third-party purity, zero dependency, public header hygiene coverage, layer violations, the forbidden graphics term, the laws-table duplication, public name collision and this very README count - was rewritten from POSIX shell to Python 3 and is now registered WITHOUT an `if(UNIX)` guard (`GATE-TREE-PARITY`, GODS_LAWS.md L-04: "o comportamento deve ser igual em qualquer OS"), so those run on all five platforms. What still does not run on Windows is the consumption and packaging family - installed consumption, embedded consumption, install layout, output name, target collision, the generated `.pc` file and its validator, blank install directory rejection - which remains POSIX shell behind that guard, plus `visibility_test` (a shared-object query with no meaning in static mode, which is why shared and static come out equal here) and the two Wayland connection cases, whose Windows counterpart is `win32_display_connect_test`, counted above. One gate is registered on Windows and DECLARES its own inapplicability there instead of vanishing: `public_name_collision_test` runs, passes, and prints `1 case, 0 exercised, 1 declared not applicable`, because the system-header discovery it needs uses GCC/Clang-only front-end options with no MSVC equivalent - a declared and COUNTED absence, not a silent skip. Outside `ctest`, `.github/workflows/ci.yml`'s `windows` job also runs dedicated PowerShell scripts (`tools/ci/check-embed.ps1` and the "Consumo instalado"/"Consumo embutido" steps, plus dependency-zero and port-privacy checks against the real compiled binary's own imports and exports) that cover, on that platform, the same consumption and boundary claims the Unix-only `ctest` gates prove through it on Linux - those checks are real and running, they are just not counted in the two numbers above because they are not `ctest` cases.
