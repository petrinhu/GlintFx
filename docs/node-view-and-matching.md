...
**What still defers through here too, on purpose:** `:placeholder-shown` (gap 1 above, unresolved by product decision) and every pseudo-element (`::before`, `::after` - fabricating the box they denote is layout's own future job). A chain carrying either one, anywhere, still comes back `deferred` from `match_complex()`, never a guessed `matched`.
