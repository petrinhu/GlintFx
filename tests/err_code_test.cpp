...
// err_code_test.cpp - CE-1 of CORE-ERROR (TODO.md, GODS_LAWS.md L-20,
// L-40): proves the whole gltfx_err_code table is internally
// consistent - every enumerator has a name, no two enumerators share a
// name, no two enumerators share a numeric value - and that a value
// outside the table degrades to "unknown" instead of undefined
// behavior.
//
// ENUMERATION, not directed search (GODS_LAWS.md L-17/L-27 lineage,
// "enumere o espaco pequeno"): k_all_codes below is the closed,
// hand-written list of every enumerator err_code.hpp declares. There
...
constexpr std::array<glintfx::gltfx_err_code, 8> k_all_codes{
    glintfx::gltfx_err_code::unknown,          glintfx::gltfx_err_code::out_of_memory,
    glintfx::gltfx_err_code::io_failure,       glintfx::gltfx_err_code::not_found,
    glintfx::gltfx_err_code::invalid_argument, glintfx::gltfx_err_code::parse_failure,
    glintfx::gltfx_err_code::unsupported,      glintfx::gltfx_err_code::platform_failure,
};
...
GLINTFX_TEST(every_code_has_a_distinct_name_and_value) {
    ...
    for (std::size_t i = 0; i < k_all_codes.size(); ++i) {
        names[i] = glintfx::gltfx_err_code_name(k_all_codes[i]);
        values[i] = static_cast<std::uint32_t>(k_all_codes[i]);
        GLINTFX_CHECK(!names[i].empty());
    }

    // gltfx_err_code::unknown (index 0) is the only enumerator allowed
    // to be named "unknown"; any OTHER entry named "unknown" means the
    ...
    std::println("err_code_test: {} enumerator(s) checked, all names and values distinct",
                 k_all_codes.size());
}
...
GLINTFX_TEST(value_outside_the_table_degrades_to_unknown) {
    ...
    // build's table has never heard of, and proves
    // gltfx_err_code_name() degrades to "unknown" instead of undefined
    // behavior.
    // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange) reason: see comment above
    const auto outside = static_cast<glintfx::gltfx_err_code>(0xFFFFFFFFU);
    GLINTFX_CHECK(glintfx::gltfx_err_code_name(outside) == std::string_view{"unknown"});
}
