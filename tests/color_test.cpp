...
//
// FIELD NAMES: red/green/blue/alpha, not the shorter r/g/b/a - see
// color.hpp's own header comment for why (docs/api-conventions.md R6's
// mechanical gate flagged the single-letter `a` against a false
// positive in a Makefile comment; this is a design choice made in
// this same review, not one of the leader's six decisions).
//
...
static_assert(offsetof(glintfx::gltfx_rgba, red) == 0,
              "red must stay the first field, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(sizeof(glintfx::gltfx_rgba::red) == sizeof(float),
              "red must stay a 32-bit float, GODS_LAWS.md L-26 (CORE-COLOR)");

static_assert(offsetof(glintfx::gltfx_rgba, green) == 4,
              "green must stay the second field, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(sizeof(glintfx::gltfx_rgba::green) == sizeof(float),
              "green must stay a 32-bit float, GODS_LAWS.md L-26 (CORE-COLOR)");

static_assert(offsetof(glintfx::gltfx_rgba, blue) == 8,
              "blue must stay the third field, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(sizeof(glintfx::gltfx_rgba::blue) == sizeof(float),
              "blue must stay a 32-bit float, GODS_LAWS.md L-26 (CORE-COLOR)");

// alpha is last: no field after it can reveal a gap via offsetof() the
...
static_assert(offsetof(glintfx::gltfx_rgba, alpha) == 12,
              "alpha must stay the fourth field, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(sizeof(glintfx::gltfx_rgba::alpha) == sizeof(float),
              "alpha must stay a 32-bit float, GODS_LAWS.md L-26 (CORE-COLOR)");
...
static_assert(offsetof(glintfx::gltfx_rgba8, red) == 0,
              "red must stay the first byte, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(offsetof(glintfx::gltfx_rgba8, green) == 1,
              "green must stay the second byte, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(offsetof(glintfx::gltfx_rgba8, blue) == 2,
              "blue must stay the third byte, GODS_LAWS.md L-26 (CORE-COLOR)");
static_assert(offsetof(glintfx::gltfx_rgba8, alpha) == 3,
              "alpha must stay the fourth byte, GODS_LAWS.md L-26 (CORE-COLOR)");
...
// SAME value cannot tell a channel-swap mutant - red<->green reordered
// - from a correct implementation; both would round-trip clean).
// red/green move in opposite directions as the input climbs, and
// blue/alpha are offset by different constants, so at almost every
// input at least three of the four channels disagree with each other.
glintfx::gltfx_rgba8 pattern_for(int byte_value) {
    return glintfx::gltfx_rgba8{
        .red = static_cast<std::uint8_t>(byte_value),
        .green = static_cast<std::uint8_t>(255 - byte_value),
        .blue = static_cast<std::uint8_t>((byte_value + 85) % 256),
        .alpha = static_cast<std::uint8_t>((byte_value + 170) % 256),
    };
}
...
GLINTFX_TEST(gltfx_rgba_srgb8_round_trip_covers_every_byte_value) {
    ...
    for (int byte_value = 0; byte_value <= 255; ++byte_value) {
        const glintfx::gltfx_rgba8 original = pattern_for(byte_value);
        ...
        GLINTFX_CHECK_EQ(back.red, original.red);
        GLINTFX_CHECK_EQ(back.green, original.green);
        GLINTFX_CHECK_EQ(back.blue, original.blue);
        GLINTFX_CHECK_EQ(back.alpha, original.alpha);
        ++checked;
    }
...
GLINTFX_TEST(gltfx_rgba_from_srgb8_is_linear_light_not_display_encoded) {
    const glintfx::gltfx_rgba mid_gray = glintfx::gltfx_rgba_from_srgb8(
        glintfx::gltfx_rgba8{.red = 128, .green = 128, .blue = 128, .alpha = 255});
    ...
    GLINTFX_CHECK(std::abs(mid_gray.red - k_expected_linear) < 1e-3F);
    GLINTFX_CHECK(std::abs(mid_gray.green - k_expected_linear) < 1e-3F);
    GLINTFX_CHECK(std::abs(mid_gray.blue - k_expected_linear) < 1e-3F);
    ...
    GLINTFX_CHECK(std::abs(mid_gray.red - k_naive_identity) > 0.1F);

    // Alpha bypasses the transfer function (decision 6's own text: it
    // is a coverage fraction, not light) - straight division, exact.
    GLINTFX_CHECK_EQ(mid_gray.alpha, 1.0F);
}

GLINTFX_TEST(gltfx_rgba_srgb8_round_trip_endpoints_are_exact) {
    const glintfx::gltfx_rgba black = glintfx::gltfx_rgba_from_srgb8(
        glintfx::gltfx_rgba8{.red = 0, .green = 0, .blue = 0, .alpha = 0});
    GLINTFX_CHECK_EQ(black.red, 0.0F);
    GLINTFX_CHECK_EQ(black.green, 0.0F);
    GLINTFX_CHECK_EQ(black.blue, 0.0F);
    GLINTFX_CHECK_EQ(black.alpha, 0.0F);

    const glintfx::gltfx_rgba white = glintfx::gltfx_rgba_from_srgb8(
        glintfx::gltfx_rgba8{.red = 255, .green = 255, .blue = 255, .alpha = 255});
    GLINTFX_CHECK(std::abs(white.red - 1.0F) < 1e-5F);
    GLINTFX_CHECK(std::abs(white.green - 1.0F) < 1e-5F);
    GLINTFX_CHECK(std::abs(white.blue - 1.0F) < 1e-5F);
    GLINTFX_CHECK_EQ(white.alpha, 1.0F);
}
...
GLINTFX_TEST(gltfx_rgba_to_srgb8_clamps_light_outside_the_displayable_range) {
    constexpr glintfx::gltfx_rgba hdr{.red = 2.0F, .green = -0.5F, .blue = 0.5F, .alpha = 1.5F};
    const glintfx::gltfx_rgba8 encoded = glintfx::gltfx_rgba_to_srgb8(hdr);

    GLINTFX_CHECK_EQ(encoded.red, static_cast<std::uint8_t>(255)); // brighter than white clamps up
    GLINTFX_CHECK_EQ(encoded.green, static_cast<std::uint8_t>(0)); // negative clamps down
    ...
    GLINTFX_CHECK_EQ(encoded.blue, static_cast<std::uint8_t>(188));
    GLINTFX_CHECK_EQ(encoded.alpha,
                     static_cast<std::uint8_t>(255)); // alpha clamps too, no transfer fn
}

// PREMULTIPLY SLICE: decision 6's second half, the helper decision 5's
// own text names. gltfx_rgba_premultiplied() multiplies red/green/blue
// by alpha and leaves alpha itself unchanged - a straight-line
// computation (no transfer function, no clamp), so exact equality is
// the right check, not a tolerance.

GLINTFX_TEST(gltfx_rgba_premultiplied_scales_color_channels_by_alpha_and_keeps_alpha) {
    constexpr glintfx::gltfx_rgba straight{.red = 0.8F, .green = 0.4F, .blue = 0.2F, .alpha = 0.5F};
    const glintfx::gltfx_rgba premultiplied = glintfx::gltfx_rgba_premultiplied(straight);

    GLINTFX_CHECK_EQ(premultiplied.red, 0.4F);
    GLINTFX_CHECK_EQ(premultiplied.green, 0.2F);
    GLINTFX_CHECK_EQ(premultiplied.blue, 0.1F);
    // Straight, decision 5: the helper is a TRANSIENT conversion, not
    // a second stored form - alpha itself never changes.
    GLINTFX_CHECK_EQ(premultiplied.alpha, 0.5F);
}

GLINTFX_TEST(gltfx_rgba_premultiplied_at_zero_alpha_zeroes_color_channels) {
    constexpr glintfx::gltfx_rgba fully_transparent{
        .red = 0.9F, .green = 0.6F, .blue = 0.3F, .alpha = 0.0F};
    const glintfx::gltfx_rgba premultiplied = glintfx::gltfx_rgba_premultiplied(fully_transparent);

    GLINTFX_CHECK_EQ(premultiplied.red, 0.0F);
    GLINTFX_CHECK_EQ(premultiplied.green, 0.0F);
    GLINTFX_CHECK_EQ(premultiplied.blue, 0.0F);
    GLINTFX_CHECK_EQ(premultiplied.alpha, 0.0F);
}

GLINTFX_TEST(gltfx_rgba_premultiplied_at_full_alpha_is_identity) {
    constexpr glintfx::gltfx_rgba opaque{.red = 0.7F, .green = 0.3F, .blue = 0.9F, .alpha = 1.0F};
    const glintfx::gltfx_rgba premultiplied = glintfx::gltfx_rgba_premultiplied(opaque);

    GLINTFX_CHECK_EQ(premultiplied.red, opaque.red);
    GLINTFX_CHECK_EQ(premultiplied.green, opaque.green);
    GLINTFX_CHECK_EQ(premultiplied.blue, opaque.blue);
    GLINTFX_CHECK_EQ(premultiplied.alpha, opaque.alpha);
}
