...
**Apelidos e exceções de família em N sistemas (A1c, D-A11).**
- O apelido atravessa família, nunca preenche buraco dentro dela. `X` que falta em `S` só é perdoado quando `X` é da coluna da outra família, não está registrado em nenhum sistema da família de `S`, e um parceiro de `X` da família de `S` está no inventário do próprio `S`.
- A higiene de apelidos (morto, meio-morto, bilateral, irmão vivo) é a de hoje, calculada sobre a união da família Linux contra o Windows, sem mudança.
- `missing_on=linux` expande para todos os slugs Linux; aceita-se também slug ou lista de slugs. A morte de uma exceção é por par (nome, sistema) e é relatada no sistema onde aconteceu.
- As fixturas de container que só rodam no Fedora até a A8 são declaradas como exceção com item vivo `CI-SPLIT-PER-OS`, por slug, e morrem sozinhas quando a A8 chegar.
- Linha de escopo sempre impressa: `apelidos: N linhas; perdões aplicados: fedora a, ubuntu b, ...; exceções expandidas por família: c; lacunas por sistema: ...`.

**Decisão D-A11.** O apelido atravessa FAMÍLIA, nunca preenche buraco DENTRO da família. X que falta em S só é perdoado se (i) X está na coluna da outra família; (ii) X não está em nenhum sistema da família de S; (iii) um parceiro de X da família de S está no inventário do próprio S. Mesma regra de apply_aliases() (check_test_parity.py:444-469), aplicada por sistema; a L-04 item 1 proíbe que verificação suma num sistema. Higiene de apelidos sem mudança, por família. `missing_on=linux` vale para todo slug Linux; morte de exceção por par (nome, sistema). Fixturas de container só-Fedora: exceção com item vivo CI-SPLIT-PER-OS, que morre na A8. Colateral: SISTEMAS_VALIDOS/SISTEMAS_PER_SYSTEM_ESPERADOS (check_test_parity.py:128/140) usam a fonte única de A3, com a lista de família vindo dela. Mão única: não.
...
Ordem: A0 → A1 → A1b → A1c → A2 → A3a → A3b → A3c → A4 → A5 → A6 → A7a/A7b/A7c/A7d/A7e (uma por vez) → **Parte III (B0-B3)** → A8 → A9. Um implementador por vez (todas tocam `ci.yml`). Nada de filtro por caminho (L-04 item 2). O oráculo de camadas continua ligado só nas pernas compartilhadas do servidor (L-45). **A fonte única de sistemas nasce na A3a (D-A12), antes do preparo por sistema (A3b).** Até ela, `SISTEMAS_VALIDOS`/`SISTEMAS_PER_SYSTEM_ESPERADOS`/`FAMILIA_POR_SLUG`/`_slug_family` (check_test_parity.py), `PER_SYSTEM_SLUGS_ESPERADOS`/`LINUX_FAMILY_SLUGS` (check_measured_parity.py), as tuplas `("linux","windows")` escritas à mão e as duas listas de `slug=` do job `parity` no ci.yml são duplicatas declaradas; a A3a elimina todas.
...
| A1c | Paridade por sistema: apelidos e exceções de família (D-A11) | CI | `tests/tools/check_test_parity.py` (`--per-system` aplica apelidos pelas regras (i)-(iii); expande `missing_on=linux` e listas de slugs; morte de exceção por par nome/sistema; higiene de apelidos reutilizada sobre a união da família), `tests/parity_exceptions.txt` (as fixturas de container declaradas para `ubuntu,arch,cachyos` com item `CI-SPLIT-PER-OS`) | **Vermelho real, critério fixado antes:** reprocessar os inventários do run `36085750444` (artefatos `parity-inv-*`, baixados, nenhum job pesado). Hoje: 29/54/54/54/24 lacunas (fedora/arch/cachyos/ubuntu/windows). Depois da A1c e antes das exceções de container: **exatamente** 0 no fedora, 0 no windows, e as mesmas fixturas de container em cada um de arch, cachyos e ubuntu, nada além. Depois das exceções: 0 em todos. **Vermelho de autoteste:** apelido `L\|W` com L em fedora e ubuntu e W em windows; hoje reporta W faltando em fedora/ubuntu e L em windows; depois, nada. Mutantes que têm de morrer: (M1) perdão sem a regra (ii): W registrado também no fedora (bilateral declarado) e ausente no ubuntu, que tem L, tem de sair lacuna no ubuntu; (M2) parceiro procurado na união da família em vez do próprio S: L presente só no fedora, e o ubuntu tem de reportar L E W; (M3) `linux` sem expansão: `X\|linux` com X só no windows reprova no fedora; (M4) higiene por sistema em vez de por família: um apelido normal tem de dar zero achado de higiene; (M5) morte de exceção por família inteira: `X\|linux` com X presente só no fedora tem de sair morta no fedora e viva no ubuntu | `test_parity_selftest`, `parity_textual_test` | as mesmas fixturas, com `windows` como sistema da lista | o próprio portão |
| A2 | Passos que não se pulam, inventário declarado antes, resultado agregado | CI | `.github/workflows/ci.yml` (todo passo de teste com `!cancelled()` + pré-requisito; `echo` antes do teste; `results.tsv`; passo agregado; publicação `!cancelled()`), `tests/tools/check_container_fixture_inventory.py` (aceita o `echo` antes do `exec`), portão novo `tests/tools/check_ci_step_independence.py`, `tests/tools/check_test_parity.py` (cabeçalho de completude `STATUS: completo`/`incompleto`, seção 4.2 acima) | C1 e C4 num PR com uma fixtura sabotada: hoje, 22 pulados e `parity` vermelho por "faltando"; depois, 0 pulados, agregado `falharam: 1`, `parity` verde em cobertura, job vermelho. Mutantes do portão novo: tirar `!cancelled()` de um passo → reprova; passo de teste sem pré-requisito nomeado → reprova; publicação do inventário sem `!cancelled()` → reprova | `ci_step_independence_selftest`, `container_fixture_inventory_selftest`, `container_fixture_inventory_test` | as mesmas regras nos jobs `windows*` (o portão lê o arquivo inteiro) | inalterado |
| A3a | Fonte única de sistemas (D-A12) | CI | `tools/ci/systems.txt` (`slug\|familia`, uma linha por alvo; cabeçalho: mudança só por decisão do líder, L-04 e `ESCOPO.md` §1), `tests/tools/ci_systems.py` (carrega e valida: slug `[a-z0-9-]+`, família `linux` ou `windows`, sem slug repetido, `linux` proibido como slug; um slug igual ao nome de uma família só é aceito se pertencer a ESSA família e for o único sistema dela (hoje, `windows|windows`); no dia em que a família `windows` ganhar um segundo sistema, o carregador reprova até o slug `windows` ser renomeado, e a decisão é tomada ali, não antes; pelo menos um sistema por família, arquivo vazio reprova), `tests/tools/check_test_parity.py` e `tests/tools/check_measured_parity.py` (importam o carregador; somem as cópias de :128/:140/:724/:731/:1302-1305/:1327-1328 e de :574/:698/:708/:716; slug desconhecido é erro, nunca `linux` por omissão), `.github/workflows/ci.yml` (o passo do `parity` monta os argumentos por sistema lendo `systems.txt`; a matriz do job `windows` ganha `slug: windows`), portão novo `tests/tools/check_ci_systems_source.py` (a união dos `slug` da matriz do job `linux`, cada um nos dois modos, mais o do job `windows`, é IGUAL ao conjunto de `systems.txt` com a família certa; nenhum passo `run:` do job `parity` cita slug escrito à mão) | **Vermelho de estreia (autoteste, formato já existente):** um `ci.yml` de fixtura com a matriz `{fedora, ubuntu, arch}` e um `systems.txt` de fixtura `{fedora, ubuntu, arch, cachyos}`. Hoje nenhum portão confere isso (vermelho por ausência); depois reprova citando `cachyos`. **Vermelho de comportamento:** `_slug_family("rocky-9")` devolve `linux` em silêncio (check_test_parity.py:1327-1328); depois é erro nomeando o slug. **Critério de equivalência, fixado antes:** reprocessar os inventários e medidas do run `36085750444` (como na A1c) com o portão de antes e o de depois; a saída dos dois modos, `--compare` e `--per-system`, tem de ser **idêntica** (refatoração sem mudança de comportamento), e a contagem de cópias escritas à mão nas linhas listadas cai para zero, conferida por grep nas mesmas linhas. **Mutantes que têm de morrer:** (M1) portão que compara só a CONTAGEM de slugs → trocar `arch` por `manjaro` na fixtura tem de reprovar; (M2) `_slug_family` volta a devolver `linux` por omissão → slug desconhecido tem de reprovar; (M3) uma lista `slug=` escrita à mão volta ao passo do `parity` → reprova; (M4) carregador aceita slug repetido → reprova; (M5) `systems.txt` vazio → reprova (L-40); (M6) arquivo sem nenhum sistema `windows` → reprova; (M7) carregador aceita `linux` como slug → reprova; (M8) `windows|windows` mais um segundo sistema da família `windows` (ex.: `windows-arm64|windows`) → reprova citando o slug `windows`; (M9) `windows|linux` (slug igual ao nome de OUTRA família) → reprova | `ci_systems_selftest`, `ci_systems_source_selftest`, `test_parity_selftest`, `check_measured_parity_selftest`, `parity_textual_test` | o job `windows` entra no mesmo portão pelo `slug: windows` da matriz | o próprio portão |
| A3b | Preparo por sistema em arquivo próprio, piso de ferramentas e uniformidade | CI | `tools/ci/env/fedora.sh`, `ubuntu.sh`, `arch.sh`, `cachyos.sh` (os 4 de hoje, **comportamento idêntico** ao `instalar` atual mais `python3` explícito), `.github/workflows/ci.yml` (matriz enxuta, passo "Piso de ferramentas"), portão novo `tests/tools/check_ci_system_uniformity.py`, `tests/tools/check_pkg_dep_coverage.py` (lê os scripts; some `MODULE_PACKAGES` com chave por distro - o pacote de cada módulo passa a ser lido do `tools/ci/env/<slug>.sh` do sistema, e os slugs vêm de `tools/ci/systems.txt` (A3a); critério fixado antes: zero chave de distro escrita à mão em check_pkg_dep_coverage.py, grep por `"(fedora|ubuntu|arch|cachyos)"` no arquivo fora dos autotestes de fixtura, e o mesmo veredito do portão antes e depois contra a árvore real) | Autoteste de uniformidade com um passo `if: matrix.nome == 'Arch - compartilhado'` fora do preparo: nenhum portão de hoje o pega (vermelho); depois reprova. Piso de ferramentas: mutante que exige `__GNUC__ >= 99` reprova nos 4 sistemas; um preparo sem `python3` reprova. Os 4 sistemas de hoje com o mesmo número de testes por perna de A0. O portão de uniformidade lê `tools/ci/systems.txt` (A3a) para os itens (3) e (4) | `ci_system_uniformity_selftest`, `pkg_dep_coverage_selftest`, `pkg_dep_coverage_test` | o Windows ganha o passo de piso equivalente (MSVC/CMake 4.1) | inalterado |
| A3c | Ordem do preparo e marcos `prep`/`build` (D-A13; conserto de A2 e A3b) | CI | `.github/workflows/ci.yml` (job `linux`: checkout de bootstrap em `_bootstrap`, preparo, remoção, checkout real, passo "Checkout é repositório git", marco `id: prep`, "Build e testes" dividido em "Build" `id: build` e "Testes"; marcos `id: prep`/`id: build` em todos os jobs da D-A13), `tools/ci/env/*.sh` (grava `GLINTFX_PREP_SHA`), `tests/tools/check_ci_step_independence.py` (G1-G3), `tests/tools/check_ci_system_uniformity.py` (G4) | Estática: MO2, que é o ci.yml de 4a3f1fd, tem de reprovar no G1; hoje nenhum portão o pega, e o run 36110716884 mostra o efeito. Dinâmica, Fedora: `git` ausente antes e presente depois de `fedora.sh`, em container descartável; mutante sem git reprova. Mutantes MO1-MO6 | `ci_step_independence_selftest`, `ci_system_uniformity_selftest`, e no servidor as 8 pernas do `linux` com a prova do git e a contagem da A0 | o job `windows` ganha os marcos; ordem inalterada (runner com git) | inalterado |
