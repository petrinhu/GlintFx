...
| [L-29](#l-29) | não saber como se implementa algo, ou querer ver prior art | Pode LER e ESTUDAR qualquer biblioteca do tipo para aprender o jeito mais rápido e eficiente; ligar, clonar e copiar são proibidos; licença conferida na fonte antes de ler |
...
**Forma confirmada pelo líder (24/09/2026, `AskUserQuestion`, depois da medição do CTO de que separar em arquivos não ganha tempo e obrigaria a paridade a juntar execuções diferentes):** ***"Espírito do astrometrica"***: um trabalho por sistema **no mesmo arquivo de CI**, e não um arquivo por sistema; a paridade continua sendo provada numa única execução.
**Decisões do líder sobre a lista (24/09/2026, `AskUserQuestion`, respostas verbatim):** o Debian 12 não tem caminho oficial para o GCC 14 → ***"Trocar por Debian 13"***; o astrometrica roda Pop!_OS e Zorin como Ubuntu 24.04 com outro rótulo, o que o item 3 desta lei proíbe tomar como suporte → ***"Imagem real de cada um"*** (montada do Ubuntu base mais o repositório oficial de cada distro; Zorin na versão 18); as análises extras (sanitizers, clang-tidy, cppcheck) → ***"Só no Fedora"***, e os testes de comportamento rodam em todas.

**Data:** 24/09/2026, verbatim: *"varra também os mesmos SO que ela varre, junto com os que ja varremos"* ("ela" = o projeto astrometrica). Depois de o main argumentar contra (compilador abaixo do piso de C++23 em Rocky 9, Debian 12 e bases Ubuntu 22.04; Wayland/KWin antigos; custo por fatia com ~14 sistemas), o líder escolheu por `AskUserQuestion`: ***"Confirmar: suporte pleno nos ~14"***. **As plataformas suportadas passam a ser as cinco originais MAIS as distros que o projeto astrometrica varre** (lista exata, lida do CI dele, no plano de `PLATFORMS-ASTRO-PARITY`; na leitura de 24/09: Debian 12, Manjaro, Mint 22, openSUSE Tumbleweed, Pop!_OS 22, Rocky 9, Ubuntu 24 e Zorin 17, além do Arch que já era alvo). **Todas com suporte pleno:** cada uma com trabalho próprio verde, e as três consequências desta lei valem para todas, sem exceção. Onde o compilador padrão da distro não atinge o piso de C++23 do projeto, o preparo do ambiente daquela distro traz o compilador (mesma ordem de 27/08: *"instale a ferramenta nos que precisarem"*); **distro que não tiver caminho para o piso volta ao líder**, que decide se ela sai. Nenhum agente tira distro da lista por conta própria.
...
3. **Nenhuma captura de foco, e nenhum dispositivo de entrada real do hospedeiro repassado ao convidado.** O caminho simplesmente não existe na forma montada, e não é criado "só para este teste". Vale o mesmo precedente do `/dev/uinput`: o que o convidado recebe do hospedeiro, ele injeta na sessão real.

   **EMENDA DO LÍDER, 07/09/2026, verbatim:** *"está liberado o teclado virtual por api, nao precisa pedir"*. **Tecla sintética entregue pela API de gerência ao teclado VIRTUAL do próprio convidado é permitida, e não precisa de autorização a cada vez.** A fronteira que separa o permitido do proibido não é "é entrada sintética?", é **por onde ela passa**: o evento que nasce na API de gerência e morre no dispositivo emulado dentro da máquina nunca toca o núcleo do sistema, nunca captura foco, nunca lê o teclado do líder, e é da mesma categoria do mouse virtual que a definição já carrega. O que continua **proibido sem exceção** é o outro caminho: repasse de dispositivo real do hospedeiro, e injeção no núcleo (o caso do `/dev/uinput`), que chega na sessão real do líder mesmo de dentro de container. Foi esse segundo caminho que travou o touchpad dele, e é ele que a lei sempre mirou.
4. **Ponte de arquivo estreita e de mão única:** o convidado enxerga apenas a árvore do projeto, montada **somente leitura**, e devolve resultado **em arquivo**. Sem área de transferência compartilhada, sem pasta do usuário, sem rede da sessão do líder.
   ...
   **EMENDA (decisão autônoma do `team-lead`, 07/09/2026, sob o modo autônomo que o líder ligou nesta onda — a confirmar por ele): a ponte de arquivo foi testada na primeira máquina real e removida.** O backend padrão que o `virt-install` usa para `--filesystem type=mount` é `virtio-9p`, e o Windows **não tem cliente 9p** para dispositivo PCI genérico (medido ao vivo: o `Dispositivo PCI` aparecia como desconhecido em `devmgmt.msc`, único item sem driver em toda a máquina). No lugar dela, o canal `org.qemu.guest_agent.0` (`virtio-serial`, anexável a quente) faz transferência de arquivo (`guest-file-write`, com contagem de bytes e assinatura conferidas nas duas pontas) e execução de comando (`guest-exec`/`guest-exec-status`, código de saída e saída padrão/erro lidos do JSON, nunca de tela) pela própria API de gerência. **Isto é MENOS superfície, não mais:** some um dispositivo (a ponte 9p não funcionava e ainda contava como exposição), nenhuma pasta do hospedeiro fica montada no convidado. O portão de isolamento (`provar-isolamento.sh`, `/var/tmp/glintfx-win-lab/`) ganhou sexta categoria — canal para o convidado que não seja este agente declarado —, provada mordendo por sabotagem antes de confiar, mesmo rito das outras cinco. Regra 4 acima permanece como o PRINCÍPIO (nenhuma pasta do usuário, nenhuma rede da sessão do líder, resultado nunca por leitura de tela); o mecanismo mudou, o princípio não.
...
**DECISÃO DO LÍDER, 24/09/2026 (`AskUserQuestion`), sobre a distância entre a letra desta lei e a prática:** o `tools/preci.sh` roda o `ctest` direto na máquina do líder (só alguns estágios usam container), e autotestes Python/shell soltos também rodavam aqui. Pergunta levada a ele; resposta, verbatim da opção escolhida: *"Containerizar o espelho (Recomendado)"*. **Consequências:** (1) o espelho e os autotestes passam a rodar dentro de container por um item próprio da tabela (`PRECI-IN-CONTAINER`); (2) **até esse item entrar**, o `preci.sh` continua rodando nesta máquina como **downgrade declarado** - todo relato que cita uma rodada do espelho diz "rodado no hospedeiro, downgrade da L-09 até PRECI-IN-CONTAINER"; (3) **nenhum teste de superfície** (janela, input, GL, compositor) roda fora do container em hipótese nenhuma, nem durante o downgrade.
...
**O que já foi PROVADO nesta fronteira, medido, não suposto (07/09/2026):** execução real de um binário do próprio GlintFx (`win32_wgl_proc_address_test`, compilado no container pelo `cl.exe` real da Microsoft — o próprio binário carrega o caminho de origem `z:/src/...` do container, prova de proveniência dentro do artefato) contra o Windows real; carga de biblioteca no runtime real da linguagem; e **mutação provada contra o sistema real pela primeira vez** — o binário mutante (mesmo teste, compilado contra código sabotado de propósito) reprovou com código de saída diferente de zero, o real passou com zero, os dois lidos do protocolo do agente (nunca de tela). Antes desta noite, verde sob reimplementação dizia só "não explodiu na imitação"; agora diz o que a lei de paridade sempre quis. Relatório completo com os números crus em `/var/tmp/glintfx-win-lab/RELATORIO.md`.

**O que esta fronteira NÃO PROVOU ainda, dito para não ficar calado:** janela real, contexto gráfico com placa de vídeo de verdade, e sanitizador. Nenhum dos três foi tentado até aqui — ausência declarada, não presumida como coberta.
...
3. **Análise estática no CI:** `clang-tidy` e `cppcheck`.
   ...
   **A análise estática no Windows roda SEMPRE em paralelo (ordem do líder, 19/09/2026: _"Nos próximos runs com Windows, paralelize a checagem estática"_ — ortografia normalizada com autorização expressa dele no mesmo dia ("voce pode consertar a ortografia!"); nenhuma palavra foi trocada, acrescentada ou retirada).** Não é preferência de desempenho, é regra de portão: medido no servidor em 19/09/2026, o passo serial custava **43m44s / 44m25s / 45m22s / 34m47s** em quatro execuções seguidas contra os poucos minutos dos outros 23 jobs — era o gargalo inteiro da execução; o mesmo passo, paralelizado, mediu **14m10s**. O grau de paralelismo se LÊ do executor em tempo de execução (`$env:NUMBER_OF_PROCESSORS`), nunca se escreve um número fixo. A armadilha que esta regra carrega junto, porque quem paralelizar de novo vai cair nela: cada iteração de `ForEach-Object -Parallel` roda num runspace PRÓPRIO — variável de fora só entra com `$using:`, e contador incrementado lá dentro **se perde em silêncio**; a contagem de cobertura (`analisados` × `encontrados`, piso da L-40) tem de vir da saída agregada do pipeline, nunca de um contador compartilhado. Vale para toda checagem estática nova que nascer num job Windows, não só para a que existe hoje.
...
**`preci.sh` antes do push.** O `TESTES.md` T15 é adotado: existe um script local que espelha o CI (formatação, configure com avisos estritos, compilação, `clang-tidy`, `cppcheck`, `ctest`) e ele roda **antes** do push, não depois. Decisão do líder de **não** amarrá-lo a hook de pre-push, para não atrasar push de documentação. **EXCEÇÃO ESTREITA, autorizada pelo líder em 10/09/2026, por `AskUserQuestion`, e ela NÃO revoga a decisão acima:** o portão de coerência de versão (`VERSION-TAG-SYNC`) pode usar um gancho de pre-push que roda **apenas quando o que está sendo enviado é uma marca de versão** (`refs/tags/v*`), ignorando envio de ramo por completo. **A razão pela qual a exceção existe:** é o único ponto que BARRA a marca errada antes de ela sair; os outros dois pontos do portão apenas auditam, e marca publicada não se desfaz sem reescrever história. **O que a exceção NÃO autoriza:** amarrar o `preci.sh`, ou qualquer trabalho pesado, a pre-push - o motivo original da decisão (não atrasar push de documentação) segue intacto, e este gancho custa milissegundos e não constrói nada. Toda ampliação além de `refs/tags/v*` exige palavra nova do líder. As ferramentas que ele exige são instalação de sistema, portanto passam pelo líder (L-14).

**Modo padrão do espelho: `--fast`. Ordem do líder, 24/09/2026:** *"nunca mais rode o preci completo sem necessidade"*. O `tools/preci.sh` sem `--fast` (que acrescenta sanitizer, depuração, montagem Windows e ligação em container, e leva várias vezes mais) só roda quando a fatia muda o que esses estágios medem - código de produto sob sanitizer, `assert()` de produto, a superfície Windows ou a ligação no container - e o motivo é escrito no briefing do agente. Para um estágio só, use o modo dele (`--lint-only`, `--sanitizer-only`), nunca a rodada inteira. Todo briefing que pede o espelho escreve o comando literal, com `--fast`: a palavra "completo" já foi lida como "sem `--fast`" e prendeu a onda numa rodada longa sem necessidade (24/09/2026).
...
**AMPLIADA em 19/09/2026, por ordem do líder**, verbatim: *"melhore a lei de 'pode ler e estudar rmlui e sdl3' para 'pode ler e estudar outras bibliotecas do tipo para aprender como fazer, sem ligar, nem clonar, apenas APRENDER o modo mais rápido e eficiente'"*. **Não é alcance novo:** a reafirmação de 17/09 (acima) já dizia *"outras libs e frameworks, incluindo rmlui e sdl3"*; o que faltava era propagar isso para o resumo da tabela de gatilhos e para o parágrafo de permissão, que continuavam nomeando só as duas — a lei se contradizia. Esta emenda termina o serviço.

**O que está PERMITIDO:** ler o código-fonte e a documentação de **qualquer biblioteca do mesmo tipo** (2D, gráfico, janela, entrada, interface, áudio, asset) para **aprender a técnica**: como um problema costuma ser atacado, que armadilhas existem, que estrutura de dados serve, e **qual é o jeito mais rápido e eficiente**. RmlUi e SDL3 continuam nomeados porque a L-43 os torna piso obrigatório de toda onda, não porque sejam teto. Aprender de prior art é engenharia normal, e a lei de dependência zero (L-07) proíbe **depender**, não proíbe **saber**.

**CONTROLE QUE VEM JUNTO DA AMPLIAÇÃO, decidido pelo líder na mesma ordem (`AskUserQuestion`, opção "Ampliar e manter o controle de licença"): antes de ler uma biblioteca ainda não liberada, CONFERIR A LICENÇA NA FONTE e registrar qual é.** A razão, e ela é concreta: o estreitamento original às duas **não era arbitrário** — carregava a verificação abaixo (MIT e zlib, conferidas no repositório delas em 21/08/2026). Ampliar sem levar o controle junto perderia essa garantia justamente onde ela custa caro: bibliotecas do mesmo domínio existem sob **copyleft forte**, ler código assim para "aprender" é terreno bem mais escorregadio que ler permissivo, e este é um repositório **público sob AGPL-3.0** com o nome do líder na autoria. Custo do controle: um minuto por biblioteca nova. Forma: uma linha no commit ou no comentário dizendo qual biblioteca foi lida e sob qual licença. **Licença não conferida = biblioteca não liberada para leitura.**
